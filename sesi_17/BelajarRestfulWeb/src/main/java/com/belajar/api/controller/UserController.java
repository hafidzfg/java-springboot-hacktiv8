...
@RestController // tag rest controller
@RequestMapping("/api/v1") // tag pembuatan endpoint
public class UserController {
	...
	@CrossOrigin
	@GetMapping("/users") // tag bikin endpoint get
	public List<User> list() {
		return userService.listAllUser();
	}

	@CrossOrigin
	@GetMapping("/users/{id}") // tag bikin endpoint get
	...
	@CrossOrigin
	@PostMapping("/users") // tag endpoint post
	public void users(@RequestBody User user) {
		userService.saveUser(user);
	}

	@CrossOrigin
	@PutMapping("/users/{id}") // tag endpoint put
